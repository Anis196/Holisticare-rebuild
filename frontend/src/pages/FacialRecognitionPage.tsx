...
const FitnessForm = () => {
  ...
  const generateWorkoutData = (goal: string): ActivityData[] => {
    ...
    try {
        ...
        // Call Spring Boot backend for plan generation
        const response = await fetch('http://localhost:8080/api/facial/recognize', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(formData)
        });
        if (!response.ok) throw new Error('Failed to get plan from backend');
        const data = await response.json();
        setPlan(data.identity || 'No plan received');
        setShowResults(true);
    } catch (err) {
